...
(function (global, factory) {
    ...
    var AnimationStyleUtil_ = anmd.__core_private__.AnimationStyleUtil;
    var AnimationStyles_ = anmd.__core_private__.AnimationStyles;
    var ANY_STATE = anmd.__core_private__.ANY_STATE;
    var EMPTY_STATE = anmd.__core_private__.EMPTY_STATE;
    ...
    var CompileAnimationStateDeclarationMetadata = (function (_super) {
        ...
        function CompileAnimationStateDeclarationMetadata(stateName, styles) {
            _super.call(this);
                ...
                if (this._assetCacheKey === UNDEFINED) {
                    if (isPresent(this.identifier)) {
                        if (isPresent(this.identifier.moduleUrl) &&
                            ...
                            var uri = anmd.reflector.importUri({
    ...
    */
    ...
    var AnimationEntryAst = (function (_super) {
        ...
        function AnimationEntryAst(name, definitions) {
            _super.call(this);
            this.name = name;
            this.definitions = definitions;
        }
    ...
    var AnimationStateTransitionAst = (function (_super) {
        ...
        function AnimationStateTransitionAst(fromState, toState, animation) {
            _super.call(this);
    ...
    function _computeLhsSetDifference(valuesA, valuesB) {
        ...
        StringMapWrapper.forEach(allValues, function (count, value) {
            if (count == -1)
                lhs.push(value);
            else if (count == 1)
                rhs.push(value);
    ...
    function _flattenStateStyles(styles) {
        ...
        styles.forEach(function (styleEntry) {
            if (isStringMap(styleEntry)) {
                StringMapWrapper.forEach(styleEntry, function (value, prop) {
    ...
    function _fillStartAndEndStateSteps(entry, fromState, toState, stateStyles, errors) {
        ...
        if (missingFromStyles.length > 0) {
            firstStepStyles.push(_populateMapFromKeyWithValue(missingFromStyles, anmd.AUTO_STYLE));
        }
        if (firstStepStyles.length > 0) {
            var firstStep = new CompileAnimationStyleMetadata(0, firstStepStyles);
        ...
        if (missingToStyles.length > 0) {
            lastStepStyles.push(_populateMapFromKeyWithValue(missingToStyles, anmd.AUTO_STYLE));
        }
                ...
                animateStyles.forEach(function (styleMetadata) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (styleEntry[0] == ':') {
                            ...
                            if (stateName == anmd.FINAL_STATE_ALIAS || stateName == toState) {
    ...
    function _parseAnimationDefinitionAnimation(entry, currentTime, collectedStyles, stateStyles, errors) {
        ...
        if (entry instanceof CompileAnimationWithStepsMetadata) {
            ...
            stylesList.forEach(function (entry) {
                StringMapWrapper.forEach(entry, function (value, prop) { collectedStyles.insertAtTime(prop, currentTime, value); });
            });
        }
        else {
    ...
    var _ANIMATION_FACTORY_RENDERER_VAR = variable('renderer');
    ...
    var _ANIMATION_FINAL_STATE_STYLES_VAR = variable('finalStateStyles');
    var _ANIMATION_STYLES_LOOKUP = variable('animationStateStyles');
    var _ANIMATION_UTIL = importExpr(Identifiers.AnimationStyleUtil);
    var _AnimationBuilder = (function () {
        ...
        _AnimationBuilder.prototype.visitAnimationStyles = function (ast, stateMap) {
            var stylesEntries = ast.styles.map(function (entry) {
                return literalMap(StringMapWrapper.keys(entry).map(function (key) { return [key, literal(entry[key])]; }));
            });
            return importExpr(Identifiers.AnimationStyles).instantiate([literalArr(stylesEntries)]);
        };
        ...
        _AnimationBuilder.prototype.visitAnimationStateTransition = function (ast, stateMap) {
            ...
            return new IfStmt(_ANIMATION_PLAYER_VAR.equals(NULL_EXPR)
        ...
        _AnimationBuilder.prototype.visitAnimationEntry = function (ast, stateMap) {
            ...
            var lookupMap = [];
            StringMapWrapper.forEach(stateMap.states, function (value, stateName) {
            ...
            statements.push(_ANIMATION_STYLES_LOOKUP.set(_ANIMATION_UTIL.instantiate([literalMap(lookupMap)])).toDeclStmt());
            ...
            statements.push(_ANIMATION_PLAYER_VAR.callMethod('onDone', [
                fn([], [
                    _ANIMATION_FACTORY_RENDERER_VAR.callMethod('setElementStyles', [
                        _ANIMATION_FACTORY_ELEMENT_VAR,
                        importExpr(Identifiers.AnimationStyleUtil).callMethod('balanceStyles', [
            ...
            statements.push(new ReturnStatement(_ANIMATION_PLAYER_VAR));
            return fn([
                new FnParam(_ANIMATION_FACTORY_RENDERER_VAR.name, importType(Identifiers.Renderer)),
        ...
        _AnimationBuilder.prototype.build = function (ast) {
            ...
            var fnVariable = variable(this.factoryName);
    ...
    function _compareToAnimationStateExpr(value, animationState) {
        if (animationState == EMPTY_STATE) {
            return value.equals(importExpr(Identifiers.uninitialized));
        }
        else if (animationState == ANY_STATE) {
            return not(value.equals(importExpr(Identifiers.uninitialized)));
        }
        else {
        ...
        boundProps.forEach(function (boundProp) {
            ...
            switch (boundProp.type) {
                ...
                case exports.PropertyBindingType.Animation:
                    ...
                    var PLAYER_VAR = variable('playerResult');
                    ...
                    updateStmts.push(THIS_EXPR.callMethod('registerActiveAnimation', [literal(animationName), PLAYER_VAR]).toStmt());
                    updateStmts.push(PLAYER_VAR.callMethod('onDone', [
                        fn([], [
                            THIS_EXPR.callMethod('registerActiveAnimation', [literal(animationName), NULL_EXPR]).toStmt()
                        ])
                    ]).toStmt());
                    updateStmts.push(PLAYER_VAR.callMethod('play', []).toStmt());
                    break;
            }
    ...
    var ViewCompiler = (function () {
        ...
        ViewCompiler.prototype.compileComponent = function (component, template, styles, pipes) {
            ...
            var statements = compiledAnimations.map(function (entry) { return entry.fnStatement; });
    ...
    var DirectiveResolver = (function () {
        function DirectiveResolver(_reflector) {
            if (isPresent(_reflector)) {
                ...
                this._reflector = anmd.reflector;
            }
        }
        /**
    ...
    var CODEGEN_DIRECTIVE_RESOLVER = new DirectiveResolver(anmd.reflector);
    ...
    var CODEGEN_PIPE_RESOLVER = new PipeResolver(anmd.reflector);
    ...
    function hasLifecycleHook(hook, token) {
        ...
        return anmd.reflector.hasLifecycleHook(token, lcInterface, lcProp);
    }
    ...
    var CompileMetadataResolver = (function () {
        ...
        CompileMetadataResolver.prototype.getAnimationMetadata = function (value) {
            ...
            if (value instanceof anmd.AnimationStyleMetadata) {
                ...
                return new CompileAnimationAnimateMetadata(value.timings, value.styles.map(function (styleEntry) { return _this.getAnimationStyleMetadata(styleEntry); }));
            }
    ...
    var StatementInterpreter = (function () {
        ...
        StatementInterpreter.prototype.visitWritePropExpr = function (expr, ctx) {
            ...
            if (isDynamicInstance(receiver)) {
                ...
                if (di.props.has(expr.name)) {
                    ...
                    anmd.reflector.setter(expr.name)(receiver, value);
                }
            }
            else {
                anmd.reflector.setter(expr.name)(receiver, value);
        ...
        StatementInterpreter.prototype.visitInvokeMethodExpr = function (expr, ctx) {
            ...
            if (isPresent(expr.builtin)) {
                ...
                if (di.methods.has(expr.name)) {
                    ...
                    result = anmd.reflector.method(expr.name)(receiver, args);
                }
            }
            else {
                result = anmd.reflector.method(expr.name)(receiver, args);
        ...
        StatementInterpreter.prototype.visitInstantiateExpr = function (ast, ctx) {
            ...
            if (clazz instanceof _DynamicClass) {
                ...
                return FunctionWrapper.apply(anmd.reflector.factory(clazz), args);
            }
        };
        ...
        StatementInterpreter.prototype.visitReadPropExpr = function (ast, ctx) {
            ...
            if (isDynamicInstance(receiver)) {
                ...
                if (di.props.has(ast.name)) {
                    ...
                    result = anmd.reflector.getter(ast.name)(receiver);
                }
            }
            else {
                result = anmd.reflector.getter(ast.name)(receiver);
